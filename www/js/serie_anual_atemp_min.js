...
var getTmp = $.getJSON('/geoserver/gcm/wfs?service=wfs&version=1.0.0&SRS=EPSG:900913&request=GetFeature&typeNames=gcm:anomalias_temp_min_anual_espacial&CQL_FILTER=BBOX%28geom,-667674,4492867,-656820,4501581%29&WIDTH=1024&HEIGHT=672&outputFormat=json', function(serie_t) {
       
				$.each(serie_t.features, function (key, val) {
				  properties = val.properties;
				  agno = properties.agno;
				  temp = properties.anomalia_temp_min_anual;
				  ix = (agno-1901);
...
$.when(getTmp).done(function() {
	...
	var chart = new Highcharts.Chart({
		...
		title: {
		    text: 'Annual min Temperature anomalies near ' + lon + ', ' + lat
		},
		subtitle: {
